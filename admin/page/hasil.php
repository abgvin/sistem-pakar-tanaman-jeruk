...
$query = mysqli_query($con, "SELECT * FROM gejala  WHERE username = '$username' ORDER BY id DESC") or die(mysqli_error($con));
// $sql = mysqli_query($con, "SELECT g.*, h.penyakit FROM gejala g, hasildiagnosis h") or die(mysqli_error($con));
$query2 = mysqli_query($con, "SELECT * FROM hasildiagnosis WHERE username = '$username' ORDER BY id DESC") or die(mysqli_error($con));
...
<div class="row">
    ...
    <h4>Hasil Diagnosa Penyakit Tanaman jeruk </h4>
    <hr>
        <h5>Gejala : </h5>
        <p> 
            <?= $j1 ?>
